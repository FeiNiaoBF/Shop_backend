...
func (s *sOrder) GetList(ctx context.Context, in model.OrderListInput) (out *model.OrderListOutput, err error) {
	...
	//  延迟初始化list切片 确定有数据，再按期望大小初始化切片容量
	out.List = make([]model.OrderListItem, 0, in.Size)
	//  把查询到的结果赋值到响应结构体中
